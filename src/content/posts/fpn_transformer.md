...
The traversal proceeds top-down. Starting at the coarsest level $L$, we compute the coarse attention logits for all block pairs $(b, c)$:

$$
    s^{(L)}[b, c] = Q^{(L)}[b] \cdot K^{(L)}[c] / \sqrt{d_h}
$$
...
This gives us a mixed-resolution attention computation: fine-grained where the model is paying attention, coarse-grained everywhere else. Every region of the sequence contributes to every query output (there is no hard zeroing) which keeps gradients alive and prevents error accumulation across layers.
...
$$
    m_{\text{new}} = \max(m, s^{(l)}[i, b])
$$
$$
    s \leftarrow s \cdot \exp(m - m_{\text{new}}) + \exp(s^{(l)}[i, b] - m_{\text{new}})
$$
$$
...
During training we replace the hard threshold decision with a soft gate, making the entire traversal differentiable. For each block pair at level $l$, the gating weight is:

$$
    g^{(l)}[b, c] = \sigma\left(\tau \cdot \left(\max_{j \in c} s^{(l)}[b, j] - \varepsilon\right)\right)
$$

where $\tau$ is a temperature parameter and $\sigma$ is the sigmoid function. At high temperature this converges to the hard threshold decision. At $\tau = 0$ the gate is always $0.5$ and the approximation has no effect, which is the behavior during the bootstrap training phase described in section 4. The output at each level is then a weighted blend of the fine and coarse representations, with $g$ controlling the mix. This is structurally similar to the soft routing used in mixture-of-experts models.
...
Each FPN applies $L$ blocks to a sequence that is halved at each level. The total cost across all levels, summed as a geometric series, is:

$$
    \sum_{l=0}^{L} \frac{n}{2^l} \cdot d_h^2 = n d_h^2 \sum_{l=0}^{L} \frac{1}{2^l} \leq 2 n d_h^2
$$

The pyramid costs $O(n d_h^2)$ regardless of $L$, the same asymptotic order as the linear projections $W_Q$, $W_K$, $W_V$ themselves. The FPN is never the bottleneck.
...
In the realistic case, if each query attends to $O(n^\alpha)$ tokens for some $0 < \alpha < 1$, the traversal costs $O(n^{1 + \alpha} d_h)$. For $\alpha = 0.5$ this is $O(n^{1.5} d_h)$, which is the complexity commonly cited for hierarchical attention schemes and matches what we observe empirically on natural language tasks.

The crossover point where the FPN overhead becomes negligible relative to the attention savings is when $n > d_h$, which holds for virtually all practical configurations ($d_h$ is typically 64-128, and the savings become significant well before that).
...
Not all transformer layers are equally good candidates for FPN injection. Attention behavior varies systematically with depth in a way that is well documented in the mechanistic interpretability literature.

Early layers tend to attend locally, building positional and syntactic representations over short windows. The attention patterns here are already dense and short-range, which means the pyramid has little to prune and the approximation error would be concentrated precisely where the model is doing the most precise work. These layers are better left untouched.

Middle layers show mixed local and mid-range attention, with some heads beginning to specialize. These are interesting candidates but carry more risk.
...
As a starting point we recommend injecting the top 25-33% of layers and profiling per-layer distillation loss before committing. If a given layer's FPN fails to approximate its exact attention within a reasonable tolerance, exclude it.
...
In the bootstrap phase, the temperature $\tau$ is set to 0, which means the soft gate always returns the exact attention output regardless of what the FPN produces. The FPN is trained purely to minimize a per-layer MSE loss between its approximate output and the frozen exact attention output:

$$
    \mathcal{L}_{\text{distill}} = \frac{1}{N_{\text{layers}}} \sum_l \left\| \hat{o}^{(l)} - o^{(l)}_{\text{exact}} \right\|_2^2
$$

This is a form of layer-wise knowledge distillation: the frozen model acts as a teacher and the FPN learns to replicate its attention behavior. The gradient flows only through the FPN weights, not the backbone, which keeps memory cost nearly identical to inference. On a single RTX 3090 with a frozen Gemma 2B backbone and 3-4 injected layers, we estimate 5-10k steps at batch size 2 with gradient accumulation takes a few hours at most.
