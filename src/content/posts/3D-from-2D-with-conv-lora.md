...
for rr in range(low_rank):
    for s in range(in_channels):
        tmp[rr, s] = torch.sum(
	        mid_kernel[rr, :]
	        * low_kernel[:, s].transpose(0, 1).reshape(in_channels, -1),
            dim=1
        )

for t in range(out_channels):
    for s in range(in_channels):
        kernel[t,s] = torch.sum(
	        tmp[:, s]
	        * up_kernel[t, :].transpose(0, 1).reshape(in_channels, -1),
            dim=1
        )
```
...
If we count the number of parameters, what we get is $r^2 \times k^2 + s \times r + t \times r$ for the full CP decomposition VS $s \times r \times k^2 + r \times t$ for HuggingFace's version (assuming we have a kxk kernel originally). This means that whenever $r$ is less than $s \times \frac{k^2 - 1}{k^2}$ the CP decomposition will save more parameters.
...
As an example, in the case where we have $t = s$ and 3x3 kernel, it becomes interesting to use the simple HuggingFace conv LoRA for $r$ lower than $0.9 s$, the CP decomposition for $r$ lower than $0.89 s$, and more interesting to use the CP decomposition than the simple version for $r$ lower than $0.88 s$. So in practice, the CP decomposition is almost always better given we often aim at ranks on the order of 10% of the weight dimensions for large networks.

## Getting there

Now that we have this conv LoRA schema we can push it one step further by fine-tuning a pre-trained 2D network into a 3D one. All we need is to convert the 2D kernel into a 3D one like so:

$$
\begin{pmatrix} a & b & c \\ d & e & f \\ g & h & i \\ \end{pmatrix} \longrightarrow
\begin{pmatrix}
\begin{pmatrix} 0 & 0 & 0 \\ 0 & 0 & 0 \\ 0 & 0 & 0 \\ \end{pmatrix} \begin{pmatrix} a & b & c \\ d & e & f \\ g & h & i \\\end{pmatrix}
...
The idea here, when moving to volumetric diffusion or video diffusion, would be to start with a pretrained SoTA image diffusion model and fine-tune some of its layers with LoRA to reap the benefits of the original costly pre-training. This mostly applies to the outer VAE of the latent diffusion process and possibly to the diffusion UNet, assuming the diffusion model itself is convolution-based. DiTs add extra complexity, since their embeddings must be modified to account for the third dimension, which this approach does not address.
