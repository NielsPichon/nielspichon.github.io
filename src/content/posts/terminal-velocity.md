...
*TLDR;* the authors propose predicting the net displacement $f_\theta(x_t, t, s)$ between 2 arbitrary timesteps $t$ and $s$ on which the model is conditioned. They derive a specific loss for this task and show that minimizing it also minimizes the distance between the distribution push-forward from $p_t$ via $f_\theta(x_t, t, 0)$ and $p_0$, up to a constant (implicit score matching formulation), which means that this formulation should indeed lead to correct transport prediction between arbitrary timesteps and the source image. Experimentally, they show state-of-the-art 1 and 4-steps generation with the latter exhibiting a lower FID score than most many-step diffusion models. The name comes from the fact that they show that estimating the net displacement from $t$ to $s$ is equivalent to estimating the velocity at the $s$, the "terminal" point of the displacement.
